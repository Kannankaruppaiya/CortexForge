...
down_revision: Union[str, Sequence[str], None] = '0001a0b2c3d4'
...
def upgrade() -> None:
    ...
    with op.batch_alter_table('memories', schema=None) as batch_op:
        batch_op.add_column(sa.Column('layer', sa.String(length=10), server_default='L1', nullable=False))
        batch_op.add_column(sa.Column('freshness_score', sa.Float(), server_default='1.0', nullable=False))
        batch_op.add_column(sa.Column('source_commit', sa.String(length=64), nullable=True))
        ...
        batch_op.add_column(sa.Column('conflict_group', sa.String(length=64), nullable=True))
        batch_op.create_index('idx_mem_project_conflict', ['project_id', 'conflict_group'], unique=False)
        batch_op.create_index('idx_mem_project_layer', ['project_id', 'layer'], unique=False)
        batch_op.create_foreign_key('fk_mem_superseded_by', 'memories', ['superseded_by_id'], ['id'], ondelete='SET NULL')
        batch_op.create_foreign_key('fk_mem_supersedes', 'memories', ['supersedes_id'], ['id'], ondelete='SET NULL')
    ...
    with op.batch_alter_table('memory_evidences', schema=None) as batch_op:
        ...
        batch_op.add_column(sa.Column('snippet_hash', sa.String(length=64), nullable=True))
        batch_op.add_column(sa.Column('ast_fingerprint', sa.String(length=64), nullable=True))
        ...
        batch_op.create_foreign_key('fk_ev_symbol', 'code_entities', ['symbol_id'], ['id'], ondelete='SET NULL')
