...
def upgrade() -> None:
    ...
    op.create_index('idx_entity_project_qualified', 'code_entities', ['project_id', 'qualified_name'])
    op.create_index('idx_entity_project_file', 'code_entities', ['project_id', 'file_path'])
    op.create_index('idx_entity_project_type', 'code_entities', ['project_id', 'entity_type'])
    op.create_index('idx_entity_project_hash', 'code_entities', ['project_id', 'content_hash'])
    ...
    op.create_table(
        ...
        sa.Column('source', sa.String(length=50), nullable=False, server_default='tree_sitter'),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
    )
    op.create_index('idx_rel_project_source', 'relationships', ['project_id', 'source_entity_id', 'relationship_type'])
    op.create_index('idx_rel_project_target', 'relationships', ['project_id', 'target_entity_id', 'relationship_type'])
    ...
    op.create_table(
        ...
        sa.Column('source_type', sa.String(length=50), nullable=False, server_default='code'),
    ...
    op.create_table(
        'memory_relations',
        sa.Column('id', sa.String(length=36), primary_key=True),
        sa.Column('source_memory_id', sa.String(length=36), sa.ForeignKey('memories.id', ondelete='CASCADE'), nullable=False),
        sa.Column('target_memory_id', sa.String(length=36), sa.ForeignKey('memories.id', ondelete='CASCADE'), nullable=False),
...
def downgrade() -> None:
    for table in (
        'agent_events', 'agent_tasks', 'memory_versions', 'memory_relations',
        'memory_evidences', 'memories', 'relationships', 'code_entities',
        'repository_snapshots', 'projects',
    ):
        op.drop_table(table)
